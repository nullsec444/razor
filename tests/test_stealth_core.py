...
from stealth_core.network import Socks5hProxy, SysctlTTL, WarpController, WarpStatus, dns_leak_prevention_environment
from stealth_core.tls_client import PROFILES, TLSClient, TLSProfile, TLSRequestError
...
def test_ttl_backup_and_restore(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr("stealth_core.network.subprocess.run", fake_run)
...
def test_tls_headers_preserve_profile_order(monkeypatch) -> None:
    ...
    monkeypatch.setattr("stealth_core.tls_client._load_curl", lambda: requests)
